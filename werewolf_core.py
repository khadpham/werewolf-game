...
class Game:
    ...
    def start_game(self):
        ...
        # print(
        #     f"Number of Werewolf: {sum(1 for player in self.players if player.role in ['Werewolf', 'BB Wolf', 'Alpha Wolf'])}"
        # )
        # print(
        #     f"Number of Villagers: {sum(1 for player in self.players if player.role not in ['Werewolf', 'BB Wolf', 'Alpha Wolf'])}"
        # )

        while True:
            if self.night_phase:
                if not self.first_night_done:
                    self.first_night_setup()  # Call the setup function for the first night
    ...
    def first_night_setup(self):
        ...
        if werewolves:
            new_names = [
                input(f"Enter new name for Werewolf {i+1}: ")
                for i in range(len(werewolves))
            ]
            for werewolf, new_name in zip(werewolves, new_names):
                werewolf.name = new_name
        ...
        for player in self.players:
            ...
            if player.role == "BB Wolf" and self.bb_wolf_target is None:
                player.name = input(f"Enter new name for BB Wolf: ")
            ...
            # Alpha Wolf's turn
            if player.role == "Alpha Wolf" and not self.alpha_wolf_ability_used:
                player.name = input(f"Enter new name for Alpha Wolf: ")
                print(
            ...
            if player.role == "Harlot":
                player.name = input(f"Enter new name for Harlot: ")
            ...
            if player.role == "Seer" and not player.visited_by_harlot:
                player.name = input(f"Enter new name for Seer: ")
            ...
            if player.role == "Bodyguard" and not player.visited_by_harlot:
                player.name = input(f"Enter new name for Bodyguard: ")
            ...
            if player.role == "Hunter" and not player.visited_by_harlot:
                player.name = input(f"Enter new name for Hunter: ")
            ...
            # Witch Actions
            if player.role == "Witch" and not player.visited_by_harlot:
                player.name = input(f"Enter new name for Witch: ")
                if (
            ...
            if player.role == "Cursed":
                player.name = input(f"Enter new name for Cursed: ")
            if player.role == "Elder":
                player.name = input(f"Enter new name for Elder: ")
        ...
        if self.alpha_wolf_target and not self.alpha_wolf_target.alive:
            ...
            self.alpha_wolf_target = (
                None  # Reset alpha_wolf_target after successful conversion
            )
        ...
        for player in self.players:
            if player.alive:
                ...
                # Alpha Wolf's turn
                    ...
                    if valid_targets:
                        print(
                            f"The Werewolves' targets are: {', '.join(target_names)}."
                        )
                        while True:
                            alpha_wolf_target_input = input(
                                f"Enter the name of the player you want to turn into a Werewolf (or leave blank for no): "
